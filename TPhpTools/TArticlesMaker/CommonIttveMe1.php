...
// *           Сформировать строки меню для записей одного родителя           *
// ****************************************************************************
function ShowCaseMe($pdo,$ParentID,$PidIn,&$cLast,&$nLine,&$cli,&$lvl,$otlada,$FirstUl=' class="accordion"')
{
   // Определяем текущий уровень меню
   $lvl++; 
   // Выбираем все записи одного родителя
   ...
   if (count($table)>0) 
   {
      ...
      foreach ($table as $row)
      {
         ...
         $cLast='+li';
         ShowCaseMe($pdo,$Uid,$Pid,$cLast,$nLine,$cli,$lvl,$otlada,' class="sub-menu"'); 
         $lvl--; 
      }
      $cli="</li>\n";
      echo($cli); $cLast='-li'; 
...
function _MakeMenu($basename,$username,$password) 
{
   ...
   ShowCaseMe($pdo,1,1,$cLast,$nLine,$cli,$lvl,$otlada);
   unset($pdo);          
}
// *************************************************************************
